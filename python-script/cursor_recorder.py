...
def save_to_file(recording, refreshAmount, path = None, name = None):
	...
	with open(os.path.join(path,name) if path and name else "cursor-recorder.json", "w+") as file:
...
def main(path = None, name = None):
	...
	save_to_file(recording, refreshAmount, path=path, name=name)
