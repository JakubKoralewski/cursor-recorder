...
Compared to the 'From Two' button above, you do not have to import the .txt file to the project panel.

1. Select the composition in the project panel you want your null to be added to.
2. Click this button.`;

{
	function cursorRecorderPanel(thisObj) {
		function buildUI(thisObj) {
			...
			let group = myPanel.add("group");
			group.orientation = 'column';
			group.alignment = ['center', 'center'];
			group.alignChildren = ['center', 'center'];
			let icon = group.add("image", undefined, logo);
			icon.maximumSize = [100, 100];
			icon.alignment = ["center", "top"];

			let fromTwo = myPanel.add("group", undefined, {name: "fromTwo"});
			fromTwo.orientation = 'row';

			let fromTwoButton = fromTwo.add("button", undefined, "Create Comp from video and data", {name: "fromTwo"});
			fromTwoButton.alignment = 'fill';
			let helpFromTwo = fromTwo.add("button", undefined, "?", {name: "helpFromTwo"});
			helpFromTwo.alignment = "right";
			helpFromTwo.maximumSize = [30, 20];

			let fromFile = myPanel.add("group", undefined, {name: "fromFile"});
			fromFile.orientation = 'row';

			let fromFileButton = fromFile.add("button", undefined, "Import .txt file from file", {name: "fromFile"});
			fromFileButton.alignment = 'fill';
			let helpFromFile = fromFile.add("button", undefined, "?", {name: "helpFromFile"});
			...
			fromFileButton.onClick = function() {
...
try{
	...
	for (let name in fileMappings) {
		...
		$.writeln('name: ', name);
		$.writeln('mapping.txt: ', mapping.txt);
		$.writeln('mapping.video: ', mapping.video);
...
try{
	...
	let file = new File(File.openDialog("Choose a file containing the cursor movement data.", "Text files: *.txt", true) as any);
	
	if(!file) {
		/* User cancelled */
		return;
	}
	addCursorRecorderNull(app.project.activeItem as CompItem, file as File);
...
try {
	...
	if (dataFile.open("r")) {
		...
		myNull.name = getStringWithoutExtension(dataFile.name).concat("-null");
