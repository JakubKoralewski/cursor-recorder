...
var helpFromFileText = "This will ask you to select a file from your drive.\nCompared to the 'From Two' button above, you do not have to import the .txt file to the project panel.\n\n1. Select the composition in the project panel you want your null to be added to.\n2. Click this button.";
{
    function cursorRecorderPanel(thisObj) {
        function buildUI(thisObj) {
            ...
            var icon = group.add("image", undefined, logo);
            icon.maximumSize = [100, 100];
            icon.alignment = ["center", "top"];
            var fromTwo = myPanel.add("group", undefined, { name: "fromTwo" });
            fromTwo.orientation = 'row';
            var fromTwoButton = fromTwo.add("button", undefined, "Create Comp from video and data", { name: "fromTwo" });
            fromTwoButton.alignment = 'fill';
            var helpFromTwo = fromTwo.add("button", undefined, "?", { name: "helpFromTwo" });
            helpFromTwo.alignment = "right";
            helpFromTwo.maximumSize = [30, 20];
            var fromFile = myPanel.add("group", undefined, { name: "fromFile" });
            fromFile.orientation = 'row';
            var fromFileButton = fromFile.add("button", undefined, "Import .txt file from file", { name: "fromFile" });
            fromFileButton.alignment = 'fill';
            var helpFromFile = fromFile.add("button", undefined, "?", { name: "helpFromFile" });
...
function fromFileButtonClicked() {
    try {
        ...
        var file = new File(File.openDialog("Choose a file containing the cursor movement data.", "Text files: *.txt", true));
        if (!file) {
            return;
        }
        addCursorRecorderNull(app.project.activeItem, file);
    }
    catch (err) {
        alert("Error at line# " + err.line.toString() + "\r" + err.toString());
    }
